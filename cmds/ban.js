...
if(!message.member.hasPermission("BAN_MEMBERS") && (message.author.id !== message.guild.ownerID)){
...
if(!persona) {
   var missingArgsEmbed = new Discord.RichEmbed() 
        ...
        .setDescription('**Cmd Usage:** b?ban [@User|Id] [Reason]')
  ...
  if(persona.highestRole.comparePositionTo(message.member.highestRole) > 0 && (message.author.id !== message.guild.ownerID)){
        ...
        .setColor(`#ff0004`)
        .setTitle("Missing Permissions")
        .setDescription("The user mentioned higest role is higger than your gigest role and you can't ban him")
            return message.channel.send(memberSuperior);
}
    ...
    if(persona.highestRole.comparePositionTo(message.member.highestRole) === 0 && (persona.user.id !== message.author.id) && (message.author.id !== message.guild.ownerID)){
        ...
        .setColor(`#ff0004`)
          .setTitle("Missing Permissions")
        .setDescription('You cant ban someone with the same higest role.')
...
razon = razon+`, Banned by ${message.author.username}`
      ...
      var bannedEmbed2 = new Discord.RichEmbed() 
        .setColor('#ffffff')
        .setAuthor(message.author.username, message.author.avatarURL)
        .setTitle(`You get banned in ${message.guild.name}`)
...
persona.ban(razon)
  ...
  .setColor('#00ff25')
  .setDescription(`**${persona} get the ban hammer.**\nReason = **${razon1}**\nBanned by ${message.author}`))
} catch (error){
 return;
}
